...
def connect_ftp(host:str, port:int = FTP_PORT, usr:str = '', pwd:str = '', save_path:str = ''):
    ...
    print(f'files:\n{l_file}\ndirs:\n{l_dir}')
    print(get_txt_file(connection, 'pub'))
    save_file(connection, 'welcome.msg', f'{save_path}/pub')
    save_file(connection, 'welcome.msg', f'{save_path}/mirrors')
    for file_info in l_dir:
        file_name = get_file_name(file_info)
        print(file_name)
        save_file(connection, f'{actual_path}/{file_name}', f'{save_path}/{file_name}')
    print(list_folder(connection, ''))
...
if __name__ == '__main__':
    ...
    connect_ftp('ftp.us.debian.org', save_path='/home/diegoavila/practicas/')
